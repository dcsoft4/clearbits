...
//
// David Ching performed the following modifications:

// 1.	Removed concept of launching browser to navigate to URL;
//		instead the parent window is notified with a Click action.
//
// 2.	Invoke hyperlink when ENTER key is pressed (as well as SPACE key)
//
// 3.	Fix AdjustSize() miscalculated width and height by one pixel.
//
// 4.	Erase background with standard dialog background color; previously the
//		background brush was returned NULL to support transparency, but this results in coarse
//		looking text after a few repaints, if Cleartype is enabled on WinXP.
//
// 5.	Make colors non-static so hyperlinks can have different color characteristics.
...
// Operations
public:	
	void GetColors(HYPERLINKCOLORS& linkColors);
    ...
    void SetColors(HYPERLINKCOLORS& colors);
...
protected:
	...
	void AdjustWindow();	
	virtual void FollowLink();	// DC make it virtual
