...
CHyperLink::CHyperLink()
{
	...
	m_crVisitedColor	= RGB(0, 0, 255);	// Blue
	m_crHoverColor		= RGB(0, 0, 255);	// Blue


	// Set default styles
	//m_dwStyle = StyleUnderline|StyleAutoSize|StyleGetFocusOnClick;

	// DC - can't initially set StyleAutoSize or else PreCreateWindow() calls AdjustWindow() to resize the window
	// before we have a chance to turn the style off
	m_dwStyle = StyleUnderline|StyleGetFocusOnClick;
}
...
{
	// Return key will select dialog's default button.  But user wants to select
	// this hyperlink.  Substitute a space key so that the hyperlink is clicked; this
	// is handled in OnKeyDown()
...
void CHyperLink::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	// DC forward on so parent dialog is notified
...
void CHyperLink::OnLButtonUp(UINT nFlags, CPoint point) 
{
	...
	// DC If we don't call this, the link stays colored in the hover color
	OnKillFocus(this);
...
{	
	if ( (nChar == VK_SPACE) || (nChar == VK_RETURN) )
	{
		...
		// Notify parent that hyperlink was 'clicked'
...
void CHyperLink::SetColors(HYPERLINKCOLORS& linkColors) {
	m_crLinkColor	 = linkColors.crLink;
	m_crActiveColor	 = linkColors.crActive;
	m_crVisitedColor = linkColors.crVisited;
	m_crHoverColor	 = linkColors.crHover;
}

void CHyperLink::GetColors(HYPERLINKCOLORS& linkColors) {
	linkColors.crLink = m_crLinkColor;
	linkColors.crActive = m_crActiveColor;
	linkColors.crVisited = m_crVisitedColor;
	linkColors.crHover = m_crHoverColor;
}
...
void CHyperLink::SetWindowText(LPCTSTR lpszText)
{
	...
	if (::IsWindow(GetSafeHwnd())) {
		...
		// DC Also update the tooltip's rectangle to the new text size
	    CRect rect; 
		GetClientRect(rect);
		m_ToolTip.SetToolRect (this, TOOLTIP_ID, rect);
...
{	
	...
	int borderWidth = rcWnd.Width() - rcClient.Width() + 1;		// DC +1 for accurate size
	int borderHeight = rcWnd.Height() - rcClient.Height() + 1;	// DC +1 for accurate size
...
BOOL CHyperLink::OnEraseBkgnd(CDC* pDC)
{
	CRect rect;
	GetClientRect (&rect);
	pDC->FillSolidRect(rect, (COLORREF) GetSysColor(COLOR_BTNFACE));
